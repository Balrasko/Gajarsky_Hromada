...
class ChatService {
  ...
  private mapMembers(memberships: ChannelMembership[]): ChannelMemberDto[] {
    return memberships.map((membership) => {
      const user = membership.$preloaded.user
  ...
  public async getUserChannels(userId: string): Promise<ChannelDto[]> {
    ...
    const memberships = await ChannelMembership.query()
      .where('userId', userId)
      .preload('channel', (builder) => {
        builder
          .where('isArchived', false)
          .preload('memberships', (membershipBuilder) => {
            membershipBuilder
              .where('status', 'active')
    ...
    const channelDtos = filtered.map((membership) => {
      ...
      const members = this.mapMembers(channel.$preloaded.memberships ?? [])
  ...
  private async channelMembers(channel: Channel): Promise<ChannelMemberDto[]> {
    await channel.load('memberships', (builder) => {
      builder.where('status', 'active').preload('user')
    })
  ...
  public async sendMessage(userId: string, channelId: string, content: string): Promise<MessageDto> {
    ...
    await channel.load('memberships', (builder) => {
      builder.where('status', 'active').preload('user', (userBuilder) => userBuilder.preload('preference'))
    })
    ...
    for (const member of channel.memberships) {
      ...
      const notifyMentionsOnly = member.$preloaded.user.$preloaded.preference?.mentionsOnlyNotifications ?? false
    ...
    const sender = channel.memberships.find((m) => m.userId === userId)?.$preloaded.user ?? await this.ensureUser(userId)
    const addressedUserModel = addressedMember
      ? channel.memberships.find((m) => m.userId === addressedMember.id)?.$preloaded.user
      : null
  ...
  public async fetchMessages(userId: string, channelId: string, cursor?: string, limit = 30) {
    ...
    const messages = await Promise.all(
      sliced.map(async (record) => {
        ...
        return record.toDto(record.$preloaded.sender!, addressedUser)
      })
    )
  ...
  public async getTypingStates(userId: string, channelId: string): Promise<TypingStateDto[]> {
    ...
    return drafts.map((draft) => ({
      userId: draft.userId,
      nickName: draft.$preloaded.user.nickName,
